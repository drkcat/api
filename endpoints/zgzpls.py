import hug, requests, json
from falcon import HTTP_400
from bs4 import BeautifulSoup

@hug.get('/bus', output=hug.output_format.pretty_json, examples=["poste=1169&source=web&simplify=false", "poste=167&source=opendata"])
def get_buses(poste:int, source='web', simplify:bool=False):
    if source == 'web':
        url = 'http://www.urbanosdezaragoza.es/frm_esquemaparadatime.php?poste={}'.format(poste)
        res = requests.get(url)

        if res.status_code != 200:
            return {
                'errors': {
                    'status': HTTP_400
                 }
            }

        soup = BeautifulSoup(res.text, 'lxml')
        table = soup.find(attrs={"style": "border:1pt solid #cccccc"}).findAll('tr')
        table.pop(0)

        street = None
        lines = None
        buses = []
        nodatabuses = []

        for row in table:
            cells = row.findAll('td')
            if simplify:
                line = int(cells[0].getText())
                destination = cells[1].getText().title()
                try:
                    time = int(cells[2].getText().replace(' minutos.', ''))
                    buses.append((line, destination, time))

                except:
                    time = cells[2].getText()
                    nodatabuses.append((line, destination, time))

            else:
                line = int(cells[0].getText())
                destination = cells[1].getText().title()
                try:
                    time = cells[2].getText().replace('minutos', 'min')
                    buses.append({
                        'line': line,
                        'destination': destination,
                        'time': time
                    })

                except:
                    time = cells[2].getText()
                    nodatabuses.append({
                        'line': line,
                        'destination': destination,
                        'time': time
                    })

        if simplify:
            buses = sorted(buses, key=lambda bus: bus[2])
        else:
            buses = sorted(buses, key=lambda bus: int(bus['time'].split()[0]))
        buses.extend(nodatabuses)


    elif source == 'opendata':
        url = 'http://www.zaragoza.es/api/recurso/urbanismo-infraestructuras/transporte-urbano/poste/tuzsa-{}.json'.format(poste)
        params = {
            'srsname': 'wgs84'
        }

        data = json.loads(requests.get(url, params=params).text)

        if 'error' in data:
            return {
                'errors': {
                    'status': HTTP_400
                 }
            }

        street = data['title'].split(')')[-1].split('Lí')[0].strip().title()
        lines = data['title'].title().split(street)[-1].strip().replace('Líneas: ','')
        buses = []
        nodatabuses = []

        for destination in data['destinos']:
            try:
                time = int(destination['primero'].replace(' minutos', '').rstrip('.'))
                if simplify:
                    buses.append((
                        int(destination['linea']),
                        destination['destino'].rstrip(',').rstrip('.').title(),
                        time
                    ))

                else:
                    buses.append({
                        'line': int(destination['linea']),
                        'destination': destination['destino'].rstrip(',').rstrip('.').title(),
                        'time': '{} min.'.format(time)
                    })

            except:
                time = destination['primero'].rstrip('.').replace('cin', 'ción')
                if simplify:
                    nodatabuses.append((
                        int(destination['linea']),
                        destination['destino'].rstrip(',').rstrip('.').title(),
                        time
                    ))

                else:
                    nodatabuses.append({
                        'line': int(destination['linea']),
                        'destination': destination['destino'].rstrip(',').rstrip('.').title(),
                        'time': '{}'.format(time)
                    })

            try:
                time = int(destination['segundo'].replace(' minutos', '').rstrip('.'))
                if simplify:
                    buses.append((
                        int(destination['linea']),
                        destination['destino'].rstrip(',').rstrip('.').title(),
                        time
                    ))

                else:
                    buses.append({
                        'line': int(destination['linea']),
                        'destination': destination['destino'].rstrip(',').rstrip('.').title(),
                        'time': '{} min.'.format(time)
                    })

            except:
                time = destination['segundo'].rstrip('.').replace('cin', 'ción')
                if simplify:
                    nodatabuses.append((
                        int(destination['linea']),
                        destination['destino'].rstrip(',').rstrip('.').title(),
                        time
                    ))

                else:
                    nodatabuses.append({
                        'line': int(destination['linea']),
                        'destination': destination['destino'].rstrip(',').rstrip('.').title(),
                        'time': '{}'.format(time)
                    })

        if simplify:
            buses = sorted(buses, key=lambda bus: bus[2])
        else:
            buses = sorted(buses, key=lambda bus: int(bus['time'].split()[0]))
        buses.extend(nodatabuses)


    else:
        return {
            'errors': {
                'status': HTTP_400
             }
        }

    return {
        'poste': poste,
        'street': street,
        'lines': lines,
        'buses': buses,
        'source': source,
        'simplify': simplify
    }
